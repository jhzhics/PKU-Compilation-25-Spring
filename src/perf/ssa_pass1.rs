...
impl TempAllocator {
    pub fn new() -> Self {
        TempAllocator { nextvar: 0, _nextlabel: 0 }
    }
...
impl Pass1Context<'_> {
    pub fn new<'a>(program: &'a koopa::ir::Program, func: &'a koopa::ir::Function) -> Pass1Context<'a> {
...
pub fn pass(prog: &koopa::ir::Program, func: koopa::ir::Function) -> Pass1Func {
    ...
    if let Some(entry) = context.layout().entry_bb() {
        pass1_func.entry = Some(format!("L{}", &context.dfg().bb(entry).name().as_ref().unwrap()[1..]));
    }
    else
    {
    ...
    pass1_func.args = context.func_data().params()
        .iter()
        .map(|arg| {
            let arg_name = context.dfg().value(*arg).name().as_ref().expect("Argument should have name");
            format!("k{}", &arg_name[1..])
        })
        .collect();
    ...
    {
        let block_name = &context.dfg().bb(*bb).name().as_ref().expect("Block name should be set")[1..];
...
fn load_integer(value: &Value, block: &mut Pass1Block, context: &Pass1Context, state: &mut Pass1State) -> String{
    ...
    fn append(&self, context: &Pass1Context, block: &mut Pass1Block, state: &mut Pass1State) {
        ...
        match value_data.kind() {
            ...
            ValueKind::Store(ins) =>
            {
                ...
                {
                    if let TypeKind::Pointer(t) = context.program().borrow_value(ins.dest()).ty().kind()
                    {
                        assert!(t.is_i32(), "Store to a pointer that is not i32 is not supported in this pass");
                    }
                    else {
            ...
            ValueKind::Load(ins) =>
            {
                ...
                {
                    if let TypeKind::Pointer(t) = context.program().borrow_value(ins.src()).ty().kind()
                    {
                        assert!(t.is_i32(), "Store to a pointer that is not i32 is not supported in this pass");
                    }
                    else {
            ...
            ValueKind::Alloc(_ins) => 
            {
                if let TypeKind::Pointer(t) = value_data.ty().kind()
                {
                    if t.is_i32()
                    {
                        let value_name = value_data.name().as_ref().expect("Value should have a name");
            ...
            ValueKind::Branch(ins) =>
            {
                ...
                let false_target_name = &context.dfg().bb(false_target).name().as_ref().unwrap()[1..];
                block.push_instr(&format!("Br {}, L{}, L{}", cond_reg, true_target_name, false_target_name));
            ...
            ValueKind::Call(ins) =>
            {
                ...
                else
                {
                    ...
                    if arg_regs.is_empty()
                    {
                        ...
                        block.push_instr(&format!("Call_2 {}, {}, {}", ret_reg, func_name, arg_regs.join(", ")));
                    }
                }
            },
            ValueKind::GetElemPtr(ins) =>
            {
                let is_integer_zero = |value: &Value| {
                    if value.is_global() {
                        if let ValueKind::Integer(int_val) = context.program().borrow_value(*value).kind() {
                            int_val.value() == 0
                        } else {
                            false
                        }
