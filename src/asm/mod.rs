...
impl State {
    ...
    fn get_offset(&self, value: Value) -> i32 {
        self.sp_offset
            .get(&value)
            .expect("Get addr of a symbol that is not allocated")
            .clone()
    }
}
...
impl GenerateAsm for Function {
    fn generate_asm(&self, asm: &mut LinkedList<String>, context: &Context) {
        ...
        for (&bb, node) in context.layout().bbs() {
            asm.push_back(format!(
                "{}:",
                backend::get_label(&bb).expect("The label is not allocated")
            ));
    ...
    fn generate_ins(&self, asm: &mut LinkedList<String>, context: &Context, func_state: &State) {
        ...
        match value_data.kind() {
            ...
            ValueKind::Branch(ins) => {
                ...
                let true_label =
                    backend::get_label(&ins.true_bb()).expect("The label is not allocated");
                let false_label =
                    backend::get_label(&ins.false_bb()).expect("The label is not allocated");
            ...
            ValueKind::Call(ins) => {
                ...
                asm.push_back(format!(
                    "call {}",
                    &context.program.func(ins.callee()).name()[1..]
                ));
                if !value_data.ty().is_unit() {
