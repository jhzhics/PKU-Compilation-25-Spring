...
impl IRState {
    ...
    fn finalize(&mut self, func_data: &mut koopa_ir::FunctionData) {
        ...
        func_data
            .layout_mut()
            .bb_mut(self.current_bb.expect("Current_bb must be set"))
            .insts_mut()
            .extend(mem::take(&mut self.instrs_list));
    }
...
fn add_libfunc_declarations(program: &mut koopa_ir::Program) {
    ...
    let func = program.new_func(koopa_ir::FunctionData::new(
        "@getarray".to_string(),
        vec![koopa_ir::Type::get_pointer(koopa_ir::Type::get_i32())],
        koopa_ir::Type::get_i32(),
    ));
...
impl KoopaAppend<koopa_ir::FunctionData, koopa_ir::Value> for Number {
    fn koopa_append(
        &self,
        func_data: &mut koopa_ir::FunctionData,
        context: IRContext,
        state: &mut IRState,
    ) -> koopa_ir::Value {
        ...
        match self {
            ...
            FuncParam::Array { btype, shape, .. } => {
                let shape = shape
                    .iter()
                    .map(|exp| {
                        let constant: i32 = exp
                            .try_into()
                            .expect("ConstDecl expect a const value at compile time");
                        constant as usize
                    })
                    .collect::<Vec<_>>();
    ...
    fn global_init(&self, program: &mut koopa_ir::Program) {
                    ...
                    for const_def in const_defs {
                        match const_def {
                            DeclEntry::Scalar { ident, init_val } => {
                                ...
                                symtable::insert(
                                    ident.name.as_str(),
                                    SymValue::VarSymbol(VarSymbol::Constant(constant)),
                                );
                            }
                            DeclEntry::Array {
                                ident,
                                ...
                                let shape = shape
                                    .iter()
                                    .map(|exp| {
                                        let constant: i32 = exp.try_into().expect(
                                            "ConstDecl expect a const value at compile time",
                                        );
                                        constant as usize
                                    })
                                    .collect::<Vec<_>>();
                                ...
                                program
                                    .set_value_name(array_value, Some(format!("@{}", ident.name)));
                                symtable::insert(
                                    ident.name.as_str(),
                                    SymValue::VarSymbol(VarSymbol::Array(array_value, shape.len())),
                                );
                            }
                        }
                    }
                        ...
                        match var_def {
                            DeclEntry::Scalar { ident, init_val } => {
                                let init_val = if let None = init_val {
                                    ...
                                    let value = exp
                                        .try_into()
                                        .expect("Decl expect a const value at compile time");
                                ...
                                symtable::insert(
                                    ident.name.as_str(),
                                    SymValue::VarSymbol(VarSymbol::Variable(var)),
                                );
                            }
                            DeclEntry::Array {
                                ident,
        ...
        for func_def in &self.func_defs {
            let params = func_def
                .params
                .iter()
                .map(|param| param.get_koopa_type())
                .collect::<Vec<_>>();
            let func = program.new_func(koopa_ir::FunctionData::new(
    ...
    fn koopa_append(
        ...
        match self {
            ...
            Exp::UnaryExp {
                ...
                exp,
            } => exp.koopa_append(func_data, context, state),
            Exp::UnaryExp { unary_op, exp } => {
                ...
                let value = func_data
                    .dfg_mut()
                    .new_value()
                    .binary(unary_op.into(), zero, value);
                state.instrs_list.push_back(value);
                value
            }
            Exp::BinaryExp {
                ...
                match &binary_op {
                    BinaryOp::LogicAnd => {
                        ...
                        let if_value = func_data
                            .dfg_mut()
                            .new_value()
                            .branch(lhs_value, then_bb, next_bb);
                        state.instrs_list.push_back(if_value);
                        state.set_current_bb(then_bb, func_data);
                        let rhs_value = rhs.koopa_append(
                            func_data,
                            IRContext {
                                next_bb: Some(next_bb),
                                ..context
                            },
                            state,
                        );
                        let bool_rhs = func_data.dfg_mut().new_value().binary(
                            koopa_ir::BinaryOp::NotEq,
                            rhs_value,
                            zero,
                        );
                    ...
                    BinaryOp::LogicOr => {
                        ...
                        let next_bb = func_data.dfg_mut().new_bb().basic_block(state.get_idx());
                        func_data.layout_mut().bbs_mut().extend([then_bb, next_bb]);
                        let cond = func_data.dfg_mut().new_value().binary(
                            koopa_ir::BinaryOp::Eq,
                            lhs_value,
                            zero,
                        );
            ...
            Exp::Call { ident, args } => {
                if let SymValue::Function(func) = symtable::get(ident.name.as_str())
                    .expect("Call a function that is not declared")
                {
                    let args = args
                        .iter()
                        .map(|arg| arg.koopa_append(func_data, context, state))
                        .collect::<Vec<_>>();
...
impl InitVal {
    ...
    fn adapt_to_shape(&self, shape: &Vec<usize>) -> InitVal {
        ...
        InitVal::Array {
            init_vals: v.into_iter().map(|x| Box::new(x)).collect(),
        }
    }
    ...
    fn __adapt_to_shape(
        ...
        mut index: Vec<usize>,
        agg_shape: &[usize],
        mut aligned_idx: usize,
        shape: &[usize],
        mut i: usize,
    ) {
        let carry = |aligned_idx: &mut usize, indices: &mut Vec<usize>, shape: &[usize]| {
        ...
        if let InitVal::Array { init_vals } = self {
            for init_val in init_vals.iter() {
                match init_val.deref() {
                    ...
                    InitVal::Array { .. } => {
                        assert!(
                            aligned_idx != agg_shape.len() - 1,
                            "Initialization of array is not aligned"
                        );
                        let new_size = agg_shape[aligned_idx];
                        let new_index = vec![0; shape.len() - aligned_idx];
                        init_val.__adapt_to_shape(
                            &mut v[i..i + new_size],
                            new_index,
                            &agg_shape[aligned_idx..],
                            1,
                            &shape[aligned_idx..],
                            0,
                        );
                        i += new_size;
                        index[aligned_idx - 1] += 1;
                        carry(&mut aligned_idx, &mut index, shape);
                    }
                }
            }
...
fn aggregate_to_shape(
    init_values: Vec<Value>,
    shape: &Vec<usize>,
    program: &mut koopa_ir::Program,
) -> Value {
    fn __aggregate_to_shape(
        init_values: &[Value],
        shape: &[usize],
        program: &mut koopa_ir::Program,
    ) -> Value {
                    ...
                    match const_def {
                        ...
                        DeclEntry::Array {
                            ...
                            for i in 0..size {
                                ...
                                let pos_value = func_data
                                    .dfg_mut()
                                    .new_value()
                                    .get_ptr(addr_value, index_value);
                                let store_value =
                                    func_data.dfg_mut().new_value().store(value, pos_value);
                        ...
                        DeclEntry::Scalar { ident, init_val } => {
                            ...
                            if let Some(init_val) = init_val {
                                if let InitVal::Scalar { exp } = init_val {
                                    let value = exp.koopa_append(func_data, context, state);
                                    let store_value =
                                        func_data.dfg_mut().new_value().store(value, var);
                        ...
                        DeclEntry::Array {
                            ...
                            if let Some(init_val) = init_val {
                                ...
                                for i in 0..size {
                                    ...
                                    let index_value =
                                        func_data.dfg_mut().new_value().integer(i as i32);
                                    let pos_value = func_data
                                        .dfg_mut()
                                        .new_value()
                                        .get_ptr(addr_value, index_value);
        ...
        match self {
            Stmt::Return { exp } => match exp {
                Some(exp) => {
                    ...
                    state
                        .instrs_list
                        .push_back(dfg.new_value().ret(Some(value)));
                }
                None => {
                    state
                        .instrs_list
                        .push_back(func_data.dfg_mut().new_value().ret(None));
                }
            },
            Stmt::Assign { lval, exp } => {
                ...
                state
                    .instrs_list
                    .push_back(dfg.new_value().store(val, addr_value));
            }
            Stmt::Exp { exp } => {
                if let Some(exp) = exp {
                    let value = exp.koopa_append(func_data, context, state);
            ...
            Stmt::If {
                ...
                if let Some(else_stmt) = else_stmt {
                    ...
                    let if_value = func_data
                        .dfg_mut()
                        .new_value()
                        .branch(cond_value, then_bb, else_bb);
                    state.instrs_list.push_back(if_value);

                    state.set_current_bb(else_bb, func_data);
                    else_stmt.koopa_append(func_data, context, state);
                } else {
                    let if_value = func_data.dfg_mut().new_value().branch(
                        cond_value,
                        then_bb,
                        context.next_bb.unwrap(),
                    );
                    state.instrs_list.push_back(if_value);
                }
            ...
            Stmt::While { cond, block } => {
                ...
                func_data
                    .layout_mut()
                    .bbs_mut()
                    .extend([cond_bb, body_bb, next_bb]);
                state
                    .instrs_list
                    .push_back(func_data.dfg_mut().new_value().jump(cond_bb));
                state.set_current_bb(cond_bb, func_data);
                let cond_value = cond.koopa_append(
                    func_data,
                    IRContext {
                        next_bb: Some(next_bb),
                        ..context
                ...
                state.instrs_list.push_back(
                    func_data
                        .dfg_mut()
                        .new_value()
                        .branch(cond_value, body_bb, next_bb),
                );
                state.set_current_bb(body_bb, func_data);
                block.koopa_append(
                    func_data,
                    IRContext {
                        next_bb: Some(cond_bb),
                        out_bb: Some(next_bb),
            ...
            Stmt::Break => {
                ...
                state
                    .instrs_list
                    .push_back(func_data.dfg_mut().new_value().jump(out_bb));
            }
            Stmt::Continue => {
    ...
    fn koopa_append(
        ...
        for item in &self.block_items {
            match item {
                ...
                BlockItem::Stmt { stmt: Stmt::Break }
                | BlockItem::Stmt {
                    stmt: Stmt::Continue,
                } => {
                    item.koopa_append(func_data, context, state);
            ...
            if let Some(next_bb) = context.next_bb {
                ...
                let ret_exp = if let TypeKind::Function(_, ret_type) = func_data.ty().kind() {
                    match ret_type.kind() {
                        ...
                        koopa_ir::TypeKind::Int32 => Some(Exp::Number {
                            value: Number { value: 0 },
                        }),
...
impl KoopaAppend<koopa_ir::Program, koopa_ir::Function> for FuncDef {
    fn koopa_append(
        ...
        let func =
            symtable::get(self.ident.name.as_str()).expect("Use a function that is not declared");
        ...
        self.params.iter().enumerate().for_each(|(i, param)| {
            let alloc_value = func_data
                .dfg_mut()
                .new_value()
                .alloc(param.get_koopa_type());
            let ith_param = func_data.params()[i];
            let store_value = func_data
                .dfg_mut()
                .new_value()
                .store(ith_param, alloc_value);
            let symvalue = if let FuncParam::Array {
                btype,
                ident,
                shape,
            } = param
        ...
        self.block
            .koopa_append(func_data, IRContext::default(), state);
        state.finalize(func_data);
        symtable::pop_scope();
        func
    }
